...
function renderHostControls(gameState, me) {
  if (!me.isHost) {
    ...
    if (balDisp) {
      ...
      const c = gameState.config || { startBalance: 30000, startBonus: 4000, buyoutMultiplier: 0, timeLimitMinutes: 0, turnTimeLimitSeconds: 0 };
      balDisp.textContent = '';
      ...
      balDisp.appendChild(document.createTextNode(
        `, Průchod START: ${fmt(c.startBonus)} Kč, Odkup koní: ${c.buyoutMultiplier > 0 ? c.buyoutMultiplier + 'x' : 'Vypnuto'}, Čas: ${c.timeLimitMinutes > 0 ? c.timeLimitMinutes + ' min' : 'Bez limitu'}, Tah: ${c.turnTimeLimitSeconds > 0 ? c.turnTimeLimitSeconds + ' s' : 'Bez limitu'}`
      ));
    }
    return;
  }
...
export function initLobbyListeners(onLeave) {
  // Config inputs (host)
  ['cfg-startBal', 'cfg-startBon', 'cfg-buyout', 'cfg-timeLimit', 'cfg-turnTimeLimit'].forEach(id => {
